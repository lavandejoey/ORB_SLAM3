...
namespace ORB_SLAM3 {
    ...
    Viewer::Viewer(System *pSystem, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Tracking *pTracking,
        ...
        if (settings) { // If settings are provided, use them
            newParameterLoader(settings);
        } else {
            cv::FileStorage fSettings(strSettingPath, cv::FileStorage::READ);
            bool is_correct = ParseViewerParamFile(fSettings); // Parse the settings file
            if (!is_correct) {
                ::std::cerr << "**ERROR in the config file, the format is not correct**" << ::std::endl;
                try { throw ::std::exception(); }
                catch (exception &e) { ::std::cerr << "Exception raised: " << e.what() << ::std::endl; }
            }
        }
        mbStopTrack = false;
    }
    ...
    void Viewer::Run() {
        ...
        while (1) {
            ...
            if (mImageViewerScale != 1.f) {
                int width = int(float(toShow.cols) * mImageViewerScale);
                int height = int(float(toShow.rows) * mImageViewerScale);
